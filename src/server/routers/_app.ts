  ...
  createSession: procedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const wordsCount = await db
        ...
        .where(
          sql`${schema.lingoWordsV2.length} = ${input.settings.length} and ${schema.lingoWordsV2.freq} >= 1000 and ${schema.lingoWordsV2.capsPercentage} > 0 and ${schema.lingoWordsV2.capsPercentage} <= 0.95`,
        )
